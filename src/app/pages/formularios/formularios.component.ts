...
export class FormulariosComponent {
  ...
  // Obtiene una referencia al elemento <dialog> en la plantilla
  @ViewChild('asesoriaDialog') asesoriaDialog!: ElementRef<HTMLDialogElement>;
  ...
  /**
   * Abre el diálogo de asesoría.
   */
  openDialog(): void {
    // Resetear los estados de carga y mensajes para el formulario 2 antes de abrirlo
    this.loading2 = false;
    this.message2 = '';
    this.error2 = '';

    if (this.asesoriaDialog && this.asesoriaDialog.nativeElement) {
      this.asesoriaDialog.nativeElement.showModal();
    }
  }

  /**
   * Cierra el diálogo de asesoría.
   */
  closeDialog(): void {
    if (this.asesoriaDialog && this.asesoriaDialog.nativeElement) {
      this.asesoriaDialog.nativeElement.close();
      this.form2.reset(); // Resetear el formulario 2 al cerrar el diálogo
    }
  }
  ...
  submitForm1() {
    ...
    this.formularioService.enviarCotizacion(payload).subscribe(response => {
      ...
      if (response.error) {
        ...
        this.openDialog(); // Abrir el diálogo en caso de éxito
      }
  ...
  submitForm2() {
    ...
    if (this.form2.invalid) {
      ...
      this.error2 = 'Por favor, completa todos los campos requeridos y corrige los errores.';
      return;
    }
    ...
    this.formularioService.enviarAsesoria(this.form2.value).subscribe(response => {
      ...
      if (response.error) {
        ...
        // El diálogo se mantendrá abierto para mostrar el mensaje de éxito
        // y se cerrará automáticamente después de un breve retraso
        setTimeout(() => {
          this.closeDialog();
      ...
      // El diálogo NO se cierra, permitiendo al usuario ver el mensaje de error.
    });
  }
}
